...
import { Home, Team, Pub, Contacts, Posts } from "./containers";
...
function App() {
  ...
  useEffect(() => {
    fetch(
      "https://api.github.com/repos/ukese/siohcawebsite/contents/siohca/src/posts?ref=dev"
    )
      ...
      .then((data) => {
        const fetchPosts = data.map((file) => {
          ...
          const formattedDate = `${monthNames[month]} ${parseInt(
            day,
            10
          )}${getDaySuffix(day)}, ${year}`;
